...
import { packages, members, clients, clientHandovers, type HandoverPackage } from '@/lib/storage';
import { formatDate, HANDOVER_TYPE_LABELS, STATUS_COLORS, TYPE_COLORS, ROLE_LABELS } from '@/lib/utils';
import { Plus, AlertTriangle, CheckCircle2, Clock, Users, Building2, FolderOpen } from 'lucide-react';

export default function DashboardPage() {
  const [pkgs, setPkgs] = useState<HandoverPackage[]>([]);
  const [memberMap, setMemberMap] = useState<Record<string, string>>({});
  const [memberRoleMap, setMemberRoleMap] = useState<Record<string, string>>({});
  const [stats, setStats] = useState({ total: 0, draft: 0, inReview: 0, complete: 0, totalMembers: 0, totalClients: 0 });

  useEffect(() => {
    const all = packages.all().sort((a, b) => b.updatedAt.localeCompare(a.updatedAt));
    setPkgs(all);
      ...
      <div className="p-6">
        ...
        {pkgs.length === 0 ? (
          ...
          <div className="space-y-3">
            {pkgs.slice(0, 10).map(pkg => {
              const completedClients = pkg.clientHandoverIds.length > 0
                ? clientHandovers.all().filter(h => pkg.clientHandoverIds.includes(h.id) && h.completionPct >= 80).length
                : 0;
              return (
                <Link key={pkg.id} href={`/handovers/${pkg.id}`}
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <p className="font-semibold text-gray-900">{memberMap[pkg.teamMemberId] || 'Unknown'}</p>
                        <span className="text-xs text-gray-400">·</span>
                        <p className="text-sm text-gray-500">{memberRoleMap[pkg.teamMemberId]}</p>
                      </div>
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${TYPE_COLORS[pkg.type]}`}>{HANDOVER_TYPE_LABELS[pkg.type]}</span>
                        <span className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${STATUS_COLORS[pkg.status]}`}>{pkg.status.replace('_', ' ')}</span>
                        <span className="text-xs text-gray-400">{formatDate(pkg.createdAt)}</span>
                      </div>
                    </div>
                    <div className="text-right shrink-0 ml-4">
                      <p className="text-sm font-semibold text-gray-900">{pkg.clientHandoverIds.length} client{pkg.clientHandoverIds.length !== 1 ? 's' : ''}</p>
