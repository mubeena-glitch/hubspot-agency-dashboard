...
import { packages, members, clientHandovers, type HandoverPackage } from '@/lib/storage';
...
export default function HandoversPage() {
  ...
  const reload = () => {
    setPkgs(packages.all().sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)));
      ...
      <div className="p-6">
        ...
        {pkgs.length === 0 ? (
          ...
          <div className="space-y-3">
            {pkgs.map(pkg => {
              const allHandovers = clientHandovers.all().filter(h => pkg.clientHandoverIds.includes(h.id));
              const avgCompletion = allHandovers.length > 0
                ? Math.round(allHandovers.reduce((s, h) => s + h.completionPct, 0) / allHandovers.length)
                : 0;
              return (
                <div key={pkg.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
                  <div className="flex items-start justify-between">
                    <Link href={`/handovers/${pkg.id}`} className="flex-1 hover:opacity-80">
                      <div className="flex items-center gap-2 mb-1">
                        <p className="font-semibold text-gray-900 text-base">{memberMap[pkg.teamMemberId] || '—'}</p>
